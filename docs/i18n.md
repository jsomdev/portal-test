...
Illustrated: *The current locale is fr-FR. During initialization the language will be extracted from the locale: 'fr'. Then you can use instance.format(name) which will return the value inside `name.fr` if it exists. If not, it will return the default `name.en`* 
...
For each *Entity* (e.g. `Category`, `Product`, `Attribute`) an EntityFormatter will be created, with the entity name being the prefix to the class (e.g. `CategoryFormatter`).
