...
export async function fetchGlobalAttributeTypes(): Promise<AttributeType[]> {
  try {
    const attributeTypesResource: AttributeTypesResource = new AttributeTypesResource();

    const queryOptions: Partial<QueryOptions> = {
      selectQuery: `id,name,description,code,sortIndex`,
    };

    const data: OdataCollection<AttributeType> = await attributeTypesResource.getEntities(
      queryOptions
    );
